...
var ENTITY_CATEGORY='category';
var ENTITY_EXPENSE='expense';
var ENTITY_INCOME='income';
var test="123456";
...
var add = function(entity) {
	...
	$("select[id$=expense-category-list] > option").remove();
	$("select[id$=income-categoryincome-list] > option").remove();
	...
	if (entity == ENTITY_EXPENSE) {
		//populating the product and contact by making an ajax call
		populateSelectBox('expense-category-list', '/category');
	}
	if (entity == ENTITY_INCOME) {
		//populating the product and contact by making an ajax call
		populateSelectBox('income-categoryincome-list', '/category');
	}
...
var formValidate = function(entity){
	...
	switch(entity){
		case ENTITY_ITEM:
			...
			break;
		case ENTITY_EXPENSE:
			var valueCategory = $('#expense-category-list').val();
			var valuePrice = $('#price').val();
			
			if(valueCategory == "" || key == ""){
				showMessage('please check the key and Cateogry values in the form', entity);
				return;
			}
			if(valuePrice == "" ){
			
				showMessage('please check the Amount should be number', entity);
				return;
			}
			break;
		case ENTITY_INCOME:
			var valueCategory = $('#income-categoryincome-list').val();
			var valuePriceincome = $('#priceincome').val();
			...
			if(valuePriceincome == "" ){
				...
				showMessage('please check the Amount it should be number', entity);
				return;
			}
			break;
		default :
...
var save = function(entity) {
	...
	// collecting the field values from the form
	 ...
	 try{
	 $('#expense-category-list').reset();//='';
	 }  catch(err){}
	 try{
	  $('#income-categoryincome-list').reset();//='';
		 }  catch(err){}
}
			...
			for(var i=0;i<formElements.length;i++){
				if(formElements[i].type !="button"){
					...
					if(ele.attr('name')=="product"){						
						...
						{
						if(ele.attr('name')=="category"){						
							$("select[id$=expense-category-list] > option").remove();
							ele.append('<option value="'+eval('data.'+ele.attr('name'))+'">'+eval('data.'+ele.attr('name'))+'</option>');	
						}
						else
							{
							if(ele.attr('name')=="categoryincome"){						
								$("select[id$=income-categoryincome-list] > option").remove();
								ele.append('<option value="'+eval('data.'+ele.attr('name'))+'">'+eval('data.'+ele.attr('name'))+'</option>');	
							}
							else
		...
		if(data.length > 0){
			for (var i=0;i<data.length;i++){
				...
				case ENTITY_CATEGORY:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].description+'</td><td>'+data[i].quantity+'</td><td>'+data[i].price+'</td>';
					break;
				case ENTITY_PRODUCT:
				...
				case ENTITY_ITEM:
					...
					break;
				case ENTITY_EXPENSE:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].price+'</td><td>'+data[i].expensedate+'</td><td>'+data[i].category+'</td><td>'+data[i].datecreated+'</td>';
					break;
				case ENTITY_INCOME:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].paymentmethod+'</td><td>'+data[i].payer+'</td><td>'+data[i].priceincome+'</td><td>'+data[i].categoryincome+'</td><td>'+data[i].paymentdate+'</td><td>'+data[i].datecreated+'</td>';
					break;
				default:
					htm+=""; 
				}
				if(entity == ENTITY_INCOME)
					htm+='<td><a href="#" class="delete-entity" onclick=\'deleteEntity("'+entity+'","'+data[i].name+'","'+data[i].categoryincome+'")\'>Delete</a> | <a href="#" class="edit-entity" onclick=\'edit("'+entity+'","'+data[i].name+'")\'>Edit</a></td></tr>';
				else
					{
				if(entity == ENTITY_EXPENSE)
					htm+='<td><a href="#" class="delete-entity" onclick=\'deleteEntity("'+entity+'","'+data[i].name+'","'+data[i].category+'")\'>Delete</a> | <a href="#" class="edit-entity" onclick=\'edit("'+entity+'","'+data[i].name+'")\'>Edit</a></td></tr>';
				else
					{
				if(entity == ENTITY_ITEM)
			...
			htm+='<tr><td colspan="'+thElesLength+'">No Record found</td></tr>';
		}
